class WritingDefinitionsController < ApplicationController
  before_action :set_writing_definition, only: [:show, :edit, :update, :destroy]
  ...
  def show
    @image_element = @writing_definition.image_elements.build
  end
  ...
  def create
    ...
    @writing_definition.user = current_user
  ...
  private
    ...
    def writing_definition_params
      params.fetch(:writing_definition, {}).permit(:definition)
    end

    def require_author
      redirect_to(site_home_url) unless @writing_definition.user == current_user
    end
end
